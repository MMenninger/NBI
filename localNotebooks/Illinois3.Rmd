...
```{r setup}
source("~/R/NBI/R/convert2016.R")
nbi <- convert2016("~/R/NBI/Data/2016hwybronlyonefile.zip")
...
il <- subset(nbisp, stateCode == "IL")
ilCounties <- subset(USCensusCounties, stateCode == "IL")
il$county <- getUSCounty(il$longitude, il$latitude, dataset = "ilCounties", stateCode = "IL")
```

 What is so unique about Illinois?

Illinois is an interesting case for a couple of reasons. Bridges are not equally distributed throughout the country. There are a lot more bridges in the Northeast of the state. Fewer in the less-developed south. 
...
plot(subset(spTransform(conus, CRS("+init=epsg:5071")), stateCode == "IL"), lwd = 5, add = TRUE)
...
points(subset(il, water == 1), pch = 2, cex = .1, col = "blue")
points(subset(il, water == 0), pch = 2, cex = .3, col = "red")
...
On the other hand, bridges that do span water follow Illinois's physical geography. 
...
Why are there so many more bridges in the Northeast of the state? Let's zoom in a bit.


# Bridge Age  
  
```{r yearBuiltHistogram}
hist(il$yearBuilt, xlab = "Year Built", main = "Year Built", n = 100)
```
...
ggplot(ilBridges,  aes(x=yearBuilt))+
  geom_histogram(binwidth = 1) +
  scale_x_continuous(breaks = seq(1870, 2020, 20), limits = c(1870, 2020))+
  labs(x= "Year Built", y= "Number of Bridges", title= "Year Illinis Bridges Built")

ggplot(ilBridges,  aes(x=yearBuilt, fill = county))+
...
animation::saveHTML(for(i in min(il$yearBuilt):max(il$yearBuilt)){
        yearlyBridgePlot(df = il, year = i)
      }, 
...
colorIndex <- .bincode(il$age, breaks = breaks)
il$colorIndex <- colorIndex
plot(subset(USCensusStates, stateCode == "IL"))
points(il$longitude, il$latitude, cex = .3, pch = 17, col = ilColors[colorIndex])
...
points(subset(il, water == 0), cex = .5, pch = 17, col = ilColors[colorIndex])
```
...
plot(subset(USCensusStates, stateCode == "IL"))
points(il, pch = 2, cex = il$averageCarCount/80000)
...
breaks <- c(seq(0, log(max(il$averageCarCount)), length = 10))
colorIndex <- .bincode(log(il$averageCarCount), breaks = breaks)
plot(subset(USCensusStates, stateCode == "IL"))
points(il, pch = 17, col = trafficColors[colorIndex], cex = log(il$averageCarCount)/20)
...
meanAgeByCounty <- aggregate(data=il, age~county, FUN = mean)
...
modeYearByCounty <- aggregate(data = il, yearBuilt~county, FUN = getmode)
...
points(il, pch = 17, cex = .2, col = "green")
